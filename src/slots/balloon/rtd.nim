...
proc bindFramebuffer*(gl: GL, i: SelfContainedImage, makeDepthAndStencil: bool = true) =
    if i.framebuffer.isEmpty:
        ...
        # gl.texImage2D(gl.TEXTURE_2D, 0, gl.R32F.GLint, texWidth.GLsizei, texHeight.GLsizei, 0.GLint, gl.RED, gl.FLOAT, nil)
        # gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, texture, 0)

        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA16F.GLint, texWidth.GLsizei, texHeight.GLsizei, 0, gl.RGBA, gl.UNSIGNED_BYTE, nil)
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, texture, 0)

        # attach only as GL_OES_depth_texture

        # glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_HALF_FLOAT_OES, 0)
