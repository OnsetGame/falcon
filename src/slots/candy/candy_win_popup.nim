...
proc setReadyForClose(csv: CandyWinDialogWindow) =
    csv.countSound.stop()
    if not csv.sm.isNil:
        csv.sm.playMusic(csv.resourcePath & "candy_sound/candy_main_game_music", 0.8)
    csv.stop.removeFromSuperview()
...
proc showStage(csv: CandyWinDialogWindow, start, to: int64, win: Win, sm: SoundManager): Animation {.discardable.}  =
    ...
    var announce: FadingSound
    case win
    of Win.Bigwin:
        announce = sm.playMusic(soundPath & "candy_big_win", 0.4)
    of Win.Hugewin:
        announce = sm.playMusic(soundPath & "candy_huge_win", 0.4)
    of Win.Megawin:
        announce = sm.playMusic(soundPath & "candy_mega_win", 0.4)
    else:
        discard
    ...
    csv.countSound = sm.playSFX(soundPath & "candy_points_count")
    csv.countSound.trySetLooping(true)
...
proc createCandyWinDialogWindowAnim(parent: Node, resourcePath: string, onDestroy: proc() = nil): CandyWinDialogWindow =
    ...
    result.stop = newButton(newRect(0, 0, VIEWPORT_SIZE.width, VIEWPORT_SIZE.height))
    parent.sceneView().addSubview(result.stop)
    result.stop.hasBezel = false
...
proc showFreespinsResult*(parent: Node, payout: int64, sm: SoundManager, resourcePath: string, onDestroy: proc() = nil): CandyWinDialogWindow =
    ...
    res.countSound = sm.playSFX(resourcePath & "candy_sound/" & "candy_points_count")
    res.countSound.trySetLooping(true)
...
proc showBigwins*(parent: Node, payout, totalBet: int64, sm: SoundManager, resourcePath: string, thresholds: seq[int64], onDestroy: proc() = nil, isBonus: bool = false, closeButton: Button = nil): CandyWinDialogWindow =
    let res = createCandyWinDialogWindowAnim(parent, resourcePath, onDestroy)
