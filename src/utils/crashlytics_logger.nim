...
# jclass com.crashlytics.android.Crashlytics of JVMObject:
#     proc log(s: string) {.tags: [], `static`.}
...
type
    CrashlyticsContextObj {.importc: "crashlytics_context_t", nodecl.} = object
        log: proc(ctx: CrashlyticsContext, s: cstring) {.cdecl, tags: [], gcsafe.}
        set_user_identifier: proc(ctx: CrashlyticsContext, s: cstring) {.cdecl, tags: [].}
    CrashlyticsContext = ptr CrashlyticsContextObj

proc crashlytics_init(): CrashlyticsContext {.importc, nodecl.}

var crashlytics: CrashlyticsContext

{.push stackTrace: off.}
proc Java_com_onsetgame_reelvalley_MainActivity_initCrashlytics(env: pointer, this: pointer) {.exportc.} =
    crashlytics = crashlytics_init()
{.pop.}

type CrashlyticsLogger = ref object of Logger

proc crashlyticsLog*(s: cstring) {.exportc, gcsafe.} =
    if not crashlytics.isNil: crashlytics.log(crashlytics, s)

method log*(logger: CrashlyticsLogger, level: Level, args: varargs[string, `$`]) =
    crashlyticsLog(args.join())

proc registerLogger() =
    var lg: CrashlyticsLogger
    lg.new()
    addHandler(lg)

proc setCrashlyticsUserID*(userID: string) =
    if not crashlytics.isNil and userID.len() != 0:
        crashlytics.set_user_identifier(crashlytics, userID)
